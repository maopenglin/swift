...
+(float)height:(NSString*)text andWidth:(float)width andFont:(UIFont*)font andLineBreak:(UILineBreakMode)lineBreakMode{

    CGRect rect=CGRectMake(0, 0, width, 20000);
    NSArray *words = [[self htmlToText:text] componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    ...
    CGSize sizeSpace = [@" " sizeWithFont:font constrainedToSize:rect.size lineBreakMode:lineBreakMode];
    ...
    {
        CGSize sizeWord = [word sizeWithFont:font];
        ...
        if (![preCharacters isEqualToString:@""])
        {
            ...
            CGSize sizePreCharacters = [preCharacters sizeWithFont:font];
        ...
        if (![wordCharacters isEqualToString:@""])
        {
            ...
            CGSize sizeWordCharacters = [wordCharacters sizeWithFont:font];
        ...
        if (![postCharacters isEqualToString:@""])
        {
            ...
            if (matchNewLine)
            {
                [[postCharacters substringToIndex:matchNewLine.range.location] drawAtPoint:drawPoint withFont:font];
                drawPoint = CGPointMake(0.0, drawPoint.y + sizeWord.height);
                [[postCharacters substringFromIndex:matchNewLine.range.location + matchNewLine.range.length] drawAtPoint:drawPoint withFont:font];
                drawPoint = CGPointMake(drawPoint.x + [[postCharacters substringFromIndex:matchNewLine.range.location + matchNewLine.range.length] sizeWithFont:font].width, drawPoint.y);
            }
            else
            {
                CGSize sizePostCharacters = [postCharacters sizeWithFont:font];
...
- (void)drawTextInRect:(CGRect)rect
{
    ...
    NSArray *words = [[STTweetLabel htmlToText:self.text]  componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
